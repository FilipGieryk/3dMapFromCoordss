...
def fill_holes_with_neighbors(grid, nodata=65535, max_iter=10):
    """
    Fill nodata cells (value=nodata) with the average of their non-nodata neighbors.
    Repeats up to max_iter times or until no nodata remains.
    """
    def avg_filter(values):
        center = values[len(values)//2]
        if center != nodata:
            return center
        # Exclude nodata from neighbors
