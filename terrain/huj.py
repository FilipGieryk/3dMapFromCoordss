...
def rasterize_xyz(df, grid_size, output_folder, tile_x, tile_y, global_min_height):
    ...
    # --- Global normalization: global_min_height (deepest) = black, water_level (0) = white ---
    water_level = 0.0  # Z value for water level (white)
    if global_min_height < water_level:
        grid[mask] = ((stat[mask] - global_min_height) / (water_level - global_min_height) * 65535).astype(np.uint16)
    else:
        grid[mask] = 65535
    # ---------------------------------------------------------------------------
    ...
    # RAW
    raw_name = f"tile_{x_min:.2f}_{y_min:.2f}_{width:.2f}_{length:.2f}_{global_min_height:.2f}_0.00_rasterized{grid_size}x{grid_size}.raw"
    ...
    # TIFF
    tiff_name = f"tile_{x_min:.2f}_{y_min:.2f}_{width:.2f}_{length:.2f}_{global_min_height:.2f}_0.00_rasterized{grid_size}x{grid_size}.tif"
...
def split_and_rasterize(df, output_prefix, global_min_height):
    ...
    for ix in range(n_tiles_x):
        for iy in range(n_tiles_y):
            ...
            rasterize_xyz(tile_df, grid_size, folder, ix, iy, global_min_height)
            tile_count += 1
...
def main():
    ...
    df['z'] = df['z'] - 2.0  # Lower all heights by 2 meters
    df = relative_to_first(df, decimals=2)  # Only X and Y are made relative
    ...
    global_min_height = df['z'].min()
    split_and_rasterize(df, output_prefix, global_min_height)
